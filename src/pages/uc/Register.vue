            ...
            <div class="login_right" style="padding:20px 30px;">
                <Form v-if="allowRegister" ref="formInline" :model="formInline" :rules="ruleInline" inline>
                    ...
                    <FormItem prop="country">
                        <Select v-if="switchLang==='zh' || switchLang==='tw'" v-model="formInline.country" :placeholder="$t('common.country')" @on-change="onAreaChange">
                            <Option v-for="area in areas" :value="area.zhName" :key="area.zhName">{{area.zhName}}</Option>
                        </Select>
                        <Select v-else-if="switchLang==='en'" v-model="formInline.country" :placeholder="$t('common.country')" @on-change="onAreaChange">
                            <Option v-for="area in areas" :value="area.zhName" :key="area.enName">{{area.enName}}</Option>
                        </Select>
                    </FormItem>
                    <FormItem prop="user">
                    ...
                    <FormItem prop="code" v-show="showCode">
                        ...
                        <input id="sendCode" @click="sendCode()" type="Button" :value="sendCodeValue">
                    ...
                    <FormItem prop="agentcode">
                        <Input type="text" v-model="formInline.agentcode" :placeholder="$t('common.agentcode')">
...
import $ from 'jquery';
    export default {
        data () {
            ...
            return {
								switchLang: this.$i18n.locale,
                sendCodeValue:this.$t('common.sendCode'),
                registing:false,
                captchaObj:null,
                modal1:false,
                _captchaResult:null,
...
　　　　　　  },
        ...
        methods: {
            updateLangData(){
                ...
                this.sendCodeValue=this.$t('common.sendCode');
            },
            init(){
                this.getAreas();
                // this.initGtCaptcha();
            ...
            getAreas(){
                this.$http.post(this.host + this.api.common.area).then(response => {
                    ...
                    for(var i=0;i<this.areas.length;i++){
                        if(this.areas[i].zhName=='中国台湾'){
                            this.formInline.country = this.areas[i].zhName;
                            this.formInline.areaCode = this.areas[i].areaCode;
                        }
                    }
                });
